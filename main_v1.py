...
class DataBaseWid(BoxLayout):
    ...
    def check_memory(self):
        ...
        cursor = con.cursor()
        orden_execute = 'select * from ' + self.mainwid.name_db + ' ORDER BY ID DESC LIMIT ' + str(self.mainwid.num_rows)
...
class KPIWid(BoxLayout):
    ...
    def __init__(self, mainwid, **kwargs):
        ...
        # Ahorro objetivo
        try:
            ahorro_obj_mensual = (obj_cuenta - saldo)*30/(datetime.strptime(obj_fecha, '%d/%m/%Y')-datetime.now()).days
        ...
        # Ahorro real
        try:
            ahorro_real_mes = saldo * 30 / (datetime.strptime(fecha_max, '%d/%m/%Y') - datetime.strptime(fecha_min, '%d/%m/%Y')).days
        except ZeroDivisionError:
            ahorro_real_mes = 0
